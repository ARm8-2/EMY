...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def search_source(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None, bot):
        ...
        await ctx.send(embed=em, delete_after=30.0)

        def check(msg):
            return msg.content.isdigit() is True and msg.channel == channel or msg.content == 'cancel' or msg.content == 'Cancel '

        try:
            m = await cls.bot.wait_for('message', check=check, timeout=30.0)
...
class Music(commands.Cog):
    ...
    @commands.command(name='join', invoke_without_subcommand=True, aliases=['j', 'connect'])
    ...
    async def _leave(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.voice:
            embed = discord.Embed(title="", description='Not connected to any voice channel.',
                                  color=color)
    ...
    async def _volume(self, ctx: commands.Context, *, volume: int):
        ...
        if not ctx.voice_state.is_playing:
            embed = discord.Embed(title="", description='Nothing being played at the moment.',
                                  color=color)
            return await ctx.send(embed=embed)

        if 0 > volume > 100:
            embed = discord.Embed(title="", description='Volume must be between 0 and 100',
                                  color=color)
            return await ctx.send(embed=embed)

        ctx.voice_state.volume = volume / 100
        embed = discord.Embed(title="", description='Volume of the player set to {}%'.format(volume),
                              color=color)
    ...
    async def _forceskip(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.is_playing:
            embed = discord.Embed(title="", description='Not playing any music right now...',
                                  color=color)
            return await ctx.send(embed=embed)
    ...
    async def _queue(self, ctx: commands.Context, *, page: int = 1):
        ...
        if len(ctx.voice_state.songs) == 0:
            embed = discord.Embed(title="", description='Empty queue.',
                                  color=color)
            return await ctx.send(embed=embed)
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        if search.__contains__('&list='):
            async with ctx.typing():
                ...
                embed = discord.Embed(title="", description=f'Enqueued `{playlist.__len__()}` songs from **{playlistTitle}**',
                                      color=color)
                await ctx.send(embed=embed)
        else:
            async with ctx.typing():
                try:
                    ...
                    await ctx.voice_state.songs.put(song)
                    embed = discord.Embed(title="", description='Enqueued {}'.format(str(source)),
                                          color=color)
                    await ctx.send(embed=embed)
    ...
    async def _search(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                source = await YTDLSource.search_source(ctx, search, loop=self.bot.loop, bot=ctx.bot)
            except YTDLError as e:
                await ctx.send('An error occurred while processing this request: {}'.format(str(e)))
            else:
                if source == 'sel_invalid':
                    ...
                    embed = discord.Embed(title="", description='No song was selected in time',
                                          color=color)
                    await ctx.send(embed=embed)
                else:
                    if not ctx.voice_state.voice:
